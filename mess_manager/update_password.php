...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (!empty($_POST['mess_email']) && !empty($_POST['current_password']) && !empty($_POST['new_password'])) {
        $mess_email = $_POST['mess_email'];
        $current_password = $_POST['current_password'];
        $new_password = $_POST['new_password'];

        // Fetch the current hashed password from the mess_users table
        $stmt = $pdo->prepare("SELECT password FROM mess_users WHERE email = :email");
        $stmt->bindParam(':email', $mess_email);
        $stmt->execute();
        $user = $stmt->fetch(PDO::FETCH_ASSOC);

        // Verify the current password against the hash
        if ($user && password_verify($current_password, $user['password'])) {
            // Hash the new password
            $hashed_password = password_hash($new_password, PASSWORD_DEFAULT);

            try {
                // Update the password in the mess_users table
                $update_stmt = $pdo->prepare("UPDATE mess_users SET password = :password WHERE email = :email");
                $update_stmt->bindParam(':password', $hashed_password);
                $update_stmt->bindParam(':email', $mess_email);
                $update_stmt->execute();
...
<div class="form-container">
    ...
    <form method="POST" action="">
        <div class="form-group">
            <input type="email" class="form-control" name="mess_email" placeholder="Enter mess email" required>
        </div>
